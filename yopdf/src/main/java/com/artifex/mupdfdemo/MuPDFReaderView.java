...
public class MuPDFReaderView extends ReaderView {
    public enum Mode {Viewing, Selecting, Drawing, Marking}
    ...
    public void setTextSelectedListener(OnTextSelectedListener onTextSelectedListener) {
        this.textSelectedListener = onTextSelectedListener;
    }
        ...
        switch (mMode) {
            ...
            case Selecting:
                if (pageView != null) {
                    ...
                    if (this.textSelectedListener != null) {
                        this.textSelectedListener.onTextSelected(e1.getX(), e1.getY(), e2.getX(), e2.getY());
                    }
                }
                return true;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (mMode == Mode.Drawing && !tapDisabled) {
            float x = event.getX();
            float y = event.getY();
        ...
        if (mMode == Mode.Marking && !tapDisabled) {
            float x = event.getX();
            float y = event.getY();
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
...
//            pageView0.setDrawColor(colorDraw);
                pageView.startDraw(x, y);

//            pageView.selectText(x, y, x, y);
//            pageView.markupSelection(Annotation.Type.HIGHLIGHT);
            } else if (mMode == Mode.Marking) {
                a0 = x;
                b0 = y;
            }
        }
...
//                pageView.markupSelection(Annotation.Type.HIGHLIGHT);
    ...
    private void touch_up0(float x, float y) {
        if (mMode == Mode.Marking) {
            float dx = Math.abs(x - mX);
            float dy = Math.abs(y - mY);
            if (dx >= TOUCH_TOLERANCE || dy >= TOUCH_TOLERANCE) {
                MuPDFView pageView = (MuPDFView) getDisplayedView();
                if (pageView != null) {
                    pageView.selectText(a0, b0, x, y);
                    pageView.markupSelection(Annotation.Type.HIGHLIGHT);
                }
            }
        }
